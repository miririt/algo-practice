...
public:
	...
	OneBit operator[](const uint32_t& rb) {
		if (rb / 32 + 1 > size) expand(rb / 32 + 1);
	...
	bool operator==(const CrazyInt& rb) const {
		...
		for (uint32_t currIdx = 0; currIdx < rb.size; currIdx++) {
			if (this->byte[currIdx] != rb.byte[currIdx]) return false;
		}
		return true;
	}
	...
	bool operator>(const CrazyInt& rb) const {
		...
		if (!isNegative && rb.isNegative) return true;
		if (isNegative && !rb.isNegative) return false;
		if (isNegative && rb.isNegative) return -*this < -rb;

		if (size > rb.size) return true;
		if (size < rb.size) return false;
		if (!size && !rb.size) return false;

		for (uint32_t currIdx = rb.size - 1; currIdx > 0; currIdx--) {
			if (this->byte[currIdx] > rb.byte[currIdx]) return true;
			if (this->byte[currIdx] < rb.byte[currIdx]) return false;
		}
	...
	CrazyInt& operator-=(const CrazyInt& rb) {
		...
		while (currIdx < rb.size) {
			if (byte[currIdx] < rb.byte[currIdx]) {
				...
				byte[borrowIdx]--;
				while (--borrowIdx > currIdx) byte[borrowIdx] = UINT32_MAX;
